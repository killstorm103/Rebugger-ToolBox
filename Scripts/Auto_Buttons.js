<script>
// ---------- Auto_Buttons inline (temporary) ----------
(function() {
    function trimStr(str) { return str.replace(/^\s+|\s+$/g, ''); }
    function getCurrentPage() {
        var url = window.location.href;
        var q = url.indexOf('?'); if (q !== -1) url = url.substring(0, q);
    ...
    for (var i = 0; i < allPages.length; i++) {
        ...
        if (page.file.toLowerCase() !== currentFile.toLowerCase()) {
            buttonsHTML += '<a href="' + BASE_URL + page.file + '" class="nav-button" target="_blank">' + page.name + '</a>';
        }
    }
    ...
    navContainer.innerHTML = '<div style="background:#222; border-left:5px solid #dc3545; padding:15px; color:#fff; margin-bottom:0px;">' + buttonsHTML + '</div>';
    var marginHTML = '<div style="padding: 5px; background: #222; color: #fff; font-size: 14px;"><label for="marginInput">Header Offset (px):</label> <input type="text" id="marginInput" value="300" size="4" style="width: 60px; text-align: center;"> <button id="applyMarginBtn" style="margin-left: 5px; padding: 2px 10px; background: #dc3545; color: #fff; border: none; border-radius: 4px; cursor: pointer;">Apply</button> <span style="margin-left: 10px; font-size: 12px; color: #aaa;">(Current: <span id="marginValue">300</span>px)</span> <span style="margin-left: 15px; font-size: 15px; color: #ffa500;">Press L1 or R1 before pressing Start to stop PS3 from deleting your last inputted number</span></div>';
    ...
    if (marginInput && applyBtn && mainContainer) {
        function updateMargin(value) {
            ...
            if (isNaN(num)) return; if (num < 0) num = 0;
        ...
        marginInput.onkeypress = function(e) {
            ...
            if (!isDigit && !isCtrl) { if (e.preventDefault) e.preventDefault(); return false; }
            if (kc === 13) { updateMargin(marginInput.value); if (e.preventDefault) e.preventDefault(); return false; }
            return true;
        };
        marginInput.oninput = function() { this.value = this.value.replace(/[^0-9]/g, ''); };
